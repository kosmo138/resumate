    ...
    <main className="container flex flex-col items-center py-8">
      ...
      <div className="my-3 ml-20 flex w-full flex-col">
        <div className="mb-6 flex flex-row items-center">
          ...
          <Input className="ml-2 w-2/5" placeholder="직무명 입력" />
        </div>
        <div className="flex flex-row items-center">
          <Label className="text-1xl font-bold">지원회사</Label>
          <Input className="ml-2 mr-4 w-2/5" placeholder="지원 회사명 입력" />
        </div>
      </div>
      {letterBody &&
        letterBody.content &&
        letterBody.content.map((content: LetterContent, key: number) => {
          return <ContentForm key={key} />
        })}
      {!letterBody && <div className="text-2xl font-bold">로딩 중입니다</div>}
